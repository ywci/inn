...
inline void collector_send_request(coll_req_t *req)
{
...
void collector_check_resume(coll_req_t *req)
{
    if (collector_check_state(req)) {
        ...
        for (int i = 0; i < nr_nodes; i++) {
            if ((suspect & node_mask[i]) && (synth_get_liveness(i) == SUSPECT)) {
                session_t session = get_session(i);

                set_session(i, session + 1);
                synth_set_liveness(i, STOP);
            }
        }
        ...
        sampler_resume();
        collector_reset();
        crash_show_bitmap(">> collector << @resume", "available_nodes", available_nodes);
        crash_debug("handle->resume @idle (session=%d)", get_session(node_id));
...
void collector_handle_suspect(coll_req_t *req)
{
    for (int i = 0; i < nr_nodes; i++) {
        seq_t seq = req->seq[i];
...
void collector_handle_synchronize(coll_req_t *req, coll_range_t *range, timestamp_t *timestamps, zmsg_t *msg)
{
    seq_t end;
    seq_t start;
    int id = range->id;
...
void collector_handle_resume(coll_req_t *req)
{
    ...
    while (true) {
        ...
        if (!collector_can_ignore(req)) {
            if (collector_need_abort(req)) {
                ...
                switch(state) {
                case STATE_SUSPECT:
                    collector_handle_suspect(req);
                    break;
                case STATE_SYNC:
                    collector_handle_synchronize(req, range, timestamps, msg);
                    msg = NULL;
                    break;
                case STATE_RESUME:
                    collector_handle_resume(req);
                    break;
                default:
                    assert(0);
                }
