...
// #define BATCH_DEEP_MTX
#define BATCH_FAST_UPDATE

#define BATCH_MAX           128            // msg
#define BATCH_BUF_HIGH      100000         // msg
#define BATCH_SEND_INTV     500000         // nsec
#define BATCH_CHECK_INTV    1000000        // nsec
#define BATCH_RECYCLE_INTV  500000         // nsec
...
#ifdef BATCH_VERIFY_TIMESTAMP
        verify_timestamp(id, &timestamps[i]);
#endif
...
zmsg_t *batch_pack()
{
    ...
    batch_wrlock();
    count = batch_count;
    assert(count <= BATCH_NR_TIMESTAMPS);
    if (count) {
        size_t size = batch_pkt_header_size + count * sizeof(timestamp_t);
...
static inline void batch_do_handle(zmsg_t *msg, timestamp_t *timestamp, batch_record_t *rec, bool valid)
{
    track_enter();
    if (!rec) {
        if (valid) {
        ...
        if (valid) {
            ...
            // assert(batch_record_has_receiver(rec, node_id));
    ...
    if (valid) {
        ...
        batch_count++;
        batch_bufsz++;
        if ((batch_bufsz >= BATCH_BUF_HIGH) || (batch_count >= BATCH_MAX))
...
static inline void batch_handle(zmsg_t *msg)
{
    ...
    batch_do_handle(msg, timestamp, rec, valid);
...
{
    track_enter();
    synth_update(id, rec->timestamp, rec->msg);
    rec->visible[id] = true;
    track_exit();
}
    ...
    if (!list_empty(tail)) {
        for (pos = tail->next; pos != head; pos = pos->next) {
            ...
            if (batch_can_clean(id, rec)) {
                clean = true;
                rec->clean |= mask;
                show_cleaner(">> pass <<", id, rec);
            } else {
                show_cleaner(">> stop <<", id, rec);
                break;
            }
        }
