...
#ifdef CRASH_DEBUG
#define crash_debug(fmt, ...) log_debug("xxx crash xxx " fmt, ##__VA_ARGS__)
...
#define show_timestamp_info(str, id, timestamp) do {                  \
    if (id >= 0) {                                                    \
        if (strlen(str) > 0)                                          \
            log_info("%s timestamp=%08lx.%08lx.%lx (id=%d)", str, (unsigned long)(timestamp)->sec, (unsigned long)(timestamp)->usec, (unsigned long)(timestamp)->hid, id); \
        else                                                          \
            log_info("timestamp=%08lx.%08lx.%lx (id=%d)", (unsigned long)(timestamp)->sec, (unsigned long)(timestamp)->usec, (unsigned long)(timestamp)->hid, id); \
    } else {                                                          \
        if (strlen(str) > 0)                                          \
            log_info("%s timestamp=%08lx.%08lx.%lx", str, (unsigned long)(timestamp)->sec, (unsigned long)(timestamp)->usec, (unsigned long)(timestamp)->hid); \
        else                                                          \
            log_info("timestamp=%08lx.%08lx.%lx", (unsigned long)(timestamp)->sec, (unsigned long)(timestamp)->usec, (unsigned long)(timestamp)->hid); \
    }                                                                 \
} while (0)
...
#define show_record_details(id, rec) do {                             \
    if (log_is_valid() && rec) {                                      \
        gen_func_name();                                              \
        log_func("quorum=%d", rec->quorum);                           \
        log_func("ignore=%d", rec->ignore);                           \
...
#define show_queue(id, record, fmt, ...) do {                         \
    ...
    show_timestamp(DRAW_LONG_INDICATOR, id, record->timestamp);       \
} while (0)
#else
...
static inline void show_session(int id, int session, int cnt)
{
    ...
    if (session != sessions[id]) {
        sessions[id] = session;
        log_info(">> header={session:%d, count:%d}", session, cnt);
    }
}
...
#define show_ignore(id, rec) do {                                      \
    if (log_is_valid() && is_valid(&rec->pass[id])) {                  \
        log_enable();                                                  \
        gen_func_name();                                               \
        show_timestamp_details(get_func_name(), id, rec->timestamp);   \
        log_err("%s !!! error !!! delivered=%d quorum=%d", DRAW_INDICATOR, is_delivered(rec), rec->quorum); \
    } \
} while (0)
#else
...
#define show_providers(seqlist, suspect, providers) do {               \
    if (log_is_valid()) {                                              \
        crash_array_details(">> collector << @suspect", "seqlist", seqlist); \
        crash_bitmap_details(">> collector << @suspect", "suspect", suspect); \
        for (int _i; _i < nr_nodes; _i++) {                            \
            if (providers[_i]) {                                       \
                char str[32] = {0};                                    \
                sprintf(str, "queue[%d]->providers", _i);              \
                crash_bitmap_details(">> collector << @suspect", str, providers[_i]); \
            }                                                          \
        }                                                              \
    }                                                                  \
} while (0)
...
#ifdef SHOW_DELIVER
#define show_deliver(rec) show_timestamp_details(DRAW_DELIVER, -1, rec->timestamp)
#else
...
#ifdef SHOW_ANALYSIS
#define show_analysis() do {                                           \
    const int cand_max = 2;                                            \
    const int next_max = 16;                                           \
    gen_func_name();                                                   \
    for (int _i = 0; _i < nr_nodes; _i++) {                            \
        struct list_head *candidates = &synth_status.candidates[_i];   \
        synth_lock(_i);                                                \
        synth_mutex_lock();                                            \
            ...
            for (pos = candidates->next; pos != candidates; pos = pos->next) { \
                ...
                if (is_valid(next)) {                                  \
                    ...
                    for (p = next->next; p != next; p = p->next)       \
                        total++;                                       \
                    log_func_info("********************************************************************************************"); \
                    sprintf(name, "%s * {next->total=%d, ignore=%d, quorum=%d}", get_func_name(), total, curr->ignore, curr->quorum); \
                    ...
                    for (p = next->next; p != next; p = p->next) {     \
                        ...
                        log_func_info("<%d> %s (ignore=%d, quorum=%d, pass=%d), %s (id=%d)",  next_cnt + 1, ts_fst(), rec->ignore, rec->quorum, is_valid(&rec->pass[_i]), ts_snd(), _i); \
                        if (++next_cnt == next_max)                    \
                            break;                                     \
                    }                                                  \
                } else {                                               \
                    log_func_info("********************************************************************************************"); \
                    sprintf(name, "%s * {next->total=0, ignore=%d, quorum=%d}", get_func_name(), curr->ignore, curr->quorum); \
                    show_timestamp_info(name, _i, curr->timestamp);    \
                    show_bitmap_info(get_func_name(), "* receivers", curr->receivers); \
                    log_func_info("********************************************************************************************"); \
                }                                                      \
                for (int _j = 0; _j < nr_nodes; _j++) {                \
                    ...
                    if (prev) {                                        \
                        ...
                        log_func_info("[x] que%d=>%s (ignore=%d, quorum=%d, pass=%d), %s", _j, ts_fst(), prev->ignore, prev->quorum, is_valid(&prev->pass[_j]), ts_snd()); \
                    } else {                                           \
                        gen_ts_pair( "blk", NULL, "cand", curr);       \
                        log_func_info("[v] que%d=>%s (cand->count=%d, cand->pass=%d), %s", _j, ts_fst(), curr->count[_j], is_valid(&curr->pass[_i]), ts_snd()); \
                    }                                                  \
                }                                                      \
                for (int _j = 0; _j < nr_nodes; _j++) {                \
                    struct list_head *prev = curr->cand[_j].prev;      \
                    if (prev && (prev != &synth_status.candidates[_j])) { \
                        record_t *rec = list_entry(prev, record_t, cand[_j]); \
                        gen_ts_pair("prev", rec, "cand", curr);        \
                        log_func_info("> que%d=>%s (ignore=%d, quorum=%d, pass=%d), %s", _j, ts_fst(), rec->ignore, rec->quorum, is_valid(&rec->pass[_j]), ts_snd()); \
                    } else {                                           \
                        gen_ts_pair( "prev", NULL, "cand", curr);      \
                        log_func_info("> que%d=>%s (cand->input=%d), %s", _j, ts_fst(), is_valid(&curr->input[_j]), ts_snd()); \
                    }                                                  \
                }                                                      \
...
#else
#define show_analysis(...) do {} while (0)
#endif
