...
void verify_input(zmsg_t *msg)
{
    ...
    if (!verify(&verify_status.input, hdr, &cnt)) {
        if (!hdr->cnt)
            ...
            log_err("xxx invalid input xxx expected=%d, current=%d, hid=%x", cnt, hdr->cnt, hdr->hid);
    }
}
...
void verify_output(hdr_t *hdr)
{
    ...
    if (!verify(&verify_status.output, hdr, &cnt)) {
        if (!hdr->cnt)
            ...
            log_err("xxx invalid output xxx expected=%d, current=%d, hid=%x", cnt, hdr->cnt, hdr->hid);
    }
}


void verify_timestamp(int id, timestamp_t *timestamp)
{
    hdr_t hdr;
    int cnt = -1;

    hdr.hid = id ^ timestamp->hid;
    hdr.cnt = timestamp_counter(timestamp);
    if (!verify(&verify_status.timestamp, &hdr, &cnt))
        log_err("xxx invalid timestamp xxx expected=%d, current=%d", cnt, hdr.cnt);
}
