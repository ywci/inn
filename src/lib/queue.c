...
static inline queue_item_t *queue_new_chunk(int id, queue_item_t *queue, record_t *record)
{
    ...
    show_queue(id, record, "queue->count=%d", queue->count);
    return chunk;
}
...
static inline queue_item_t *queue_new_block(int id, queue_item_t *chunk, record_t *record)
{
    ...
    show_queue(id, record, "chunk->count=%d", chunk->count);
    return block;
}


static inline void queue_do_add_item(int id, queue_item_t *block, record_t *record)
{
    timestamp_t *timestamp = record->timestamp;
    ...
    show_queue(id, record, "block->count=%d (block=0x%llx)", block->count, (unsigned long long)block);
}


static inline void queue_add_item(int id, queue_item_t *queue, record_t *record)
{
    queue_item_t *chunk;
    queue_item_t *block;
    ...
    queue_do_add_item(id, block, record);
}
...
record_t *queue_update_prev(int id, record_t *prev, record_t *curr)
{
    ...
    for (i = prev->item_list[id].prev; i != &block->head; i = i->prev) {
        ...
        if (timestamp_compare(rec->timestamp, timestamp) < 0) {
            show_prev_str(id, curr, rec, "find prev in the same block");
            return rec;
        }
    }
    ...
    for (i = block->list.prev; i != &chunk->head; i = i->prev) {
        ...
        if (timestamp_compare(block->rec->timestamp, timestamp) < 0) {
            for (j = block->head.prev; j != &block->head; j = j->prev) {
                ...
                if (timestamp_compare(rec->timestamp, timestamp) < 0) {
                    show_prev_str(id, curr, rec, "find prev in the same chunk");
                    return rec;
                }
            }
            log_err("failed to find prev in the same chunk");
        }
    }
            ...
            for (j = chunk->head.prev; j != &chunk->head; j = j->prev) {
                ...
                if (timestamp_compare(block->rec->timestamp, timestamp) < 0) {
                    for (k = block->head.prev; k != &block->head; k = k->prev) {
                        ...
                        if (timestamp_compare(rec->timestamp, timestamp) < 0) {
                            show_prev_str(id, curr, rec, "find prev");
                            return rec;
                        }
                    }
...
static inline void queue_push_item(int id, queue_item_t *queue, record_t *record, bool earliest)
{
    ...
    queue->seq++;
    queue->length += 1;
    queue_add_item(id, queue, record);
}
...
void queue_push(int id, record_t *record, bool *earliest)
{
    ...
    if (queue->length < QUEUE_LENGTH) {
        ...
        show_queue(id, record, "queue->length=%d", queue->length);
    } else
        log_err("queue: failed to push, len=%d, seq=%d (id=%d)", queue->length, queue->seq, id);
}
...
static inline void queue_update_chunk(int id, queue_item_t *chunk, record_t *record)
{
    ...
    if (queue->rec == record) {
        ...
        queue->rec = chunk->rec;
        show_queue(id, chunk->rec, "update chunk->rec and queue->rec");
    } else
        show_queue(id, chunk->rec, "update chunk->rec");
}
    ...
    if (rec) {
        ...
        if (queue->rec == chunk->rec) {
            if (queue->count > 1) {
                ...
                queue->rec = nu->rec;
                show_queue(id, queue->rec, "queue->rec is updated, queue->count=%d, chunk->count=%d, block->count=%d", queue->count, chunk->count, block->count);
            } else {
                queue->rec = NULL;
                show_queue(id, record, "queue->rec is empty, queue->count=%d, chunk->count=%d, block->count=%d", queue->count, chunk->count, block->count);
            }
        }
        queue_del_item(block);
...
static inline void queue_do_remove_item(int id, queue_item_t *block, record_t *record)
{
    show_queue(id, record, "block->count=%d (block=0x%llx)", block->count, (unsigned long long)block);
...
static inline void queue_remove_item(int id, record_t *record)
{
    ...
    if (block->rec == record)
        ...
        queue_do_remove_item(id, block, record);
...
void queue_pop(int id, record_t *record)
{
    ...
    if (queue->length > 0) {
        queue_remove_item(id, record);
        queue->length--;
        show_queue(id, record, "queue->length=%d", queue->length);
    } else
        log_err("queue: failed to pop, len=%d, seq=%d (id=%d)", queue->length, queue->seq, id);
}
